...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("TANIKAMUSIC.plugins" + all_module)
    LOGGER("TANIKAMUSIC.plugins").info("𝐀𝐥𝐥 𝐅𝐞𝐚𝐭𝐮𝐫𝐞𝐬 𝐋𝐨𝐚𝐝𝐞𝐝 𝐁𝐚𝐛𝐲🥳...")
    await userbot.start()
    await VIP.start()
    await VIP.decorators()
    LOGGER("TANIKAMUSIC").info(
        "╔═════ஜ۩۞۩ஜ════╗\n  ♨️𝗠𝗔𝗗𝗘 𝗕𝗬 𝗠𝗥 𝗚𝗟𝗔𝗖𝗜𝗘𝗥♨️\n╚═════ஜ۩۞۩ஜ════╝"
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("TANIKAMUSIC").info("╔═════ஜ۩۞۩ஜ════╗\n  ♨️𝗠𝗔𝗗𝗘 𝗕𝗬 𝗠𝗥 𝗚𝗟𝗔𝗖𝗜𝗘𝗥♨️\n╚═════ஜ۩۞۩ஜ════╝")
